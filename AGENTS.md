...
Fill the placeholders from the current session:

- `--agent` — the agent name. Use `github-cloud-coding-agent` unless a more
  specific custom agent name applies.
- `--user` — the GitHub login of the user the task is running for.
- `--repo` — `OWNER/REPO` for this repository (`lucabol/bbva-observatory`).
- `--branch` — the branch you worked on.
- `--tools` — a comma-separated list of the main tools/actions you used during
  the task (for example `read,edit,test`).

Optional flags: `--team`, `--commit <SHA>`, `--model <MODEL>`,
`--input_tokens <N>`, `--output_tokens <N>`.

The emitter defaults to posting to
`https://bbva-observatory.azurewebsites.net/otel/v1/traces`. The dashboard host
must be on the coding agent firewall allowlist
